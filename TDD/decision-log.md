...
### Playtest feedback batch: eraser-loop import fix, black in the main palette row, overlay auto-collapse, color-pick exits eraser, peer box removed
**Date:** 2026-08-10 | **Slice:** polish (owner playtest feedback, same session as A/B/C) | **Decided by:** Owner (live playtest) | **Type:** Quick

**Decision:** (1) **Eraser loop bug root-caused:** the B2 loop never engaged because `ResourceImporterWAV`'s `edit/loop_mode` enum is OFFSET from `AudioStreamWAV.loop_mode` — importer `0=Detect, 1=DISABLED, 2=Forward`; the value 1 I set meant Disabled. Fixed to `2` (+ `compress/mode=0` PCM — set while chasing the bug, kept: safest for gapless looping, 190 KB). **Rule for the notes: verify importer-param edits with a headless `load()` probe of the imported resource, never trust the `.import` text alone.** (2) **Black joins the main palette row by swapping the greyscale family's one-click swatch** (mid-grey → black, the default brush color); greys stay in the All-colors grid. A 13th swatch was rejected: the draw screen min-width sits at 1254 of 1280 logical (session-12 measurement) — ~38 px more would re-break the right-edge cutoff. (3) **Starting to draw (stroke or fill, mouse or D-key) auto-collapses the expanded All-colors overlay** (`_stroke_begin`/`_fill_at` choke points). (4) **Picking any color while the eraser is active hops back to the brush** — new `CanvasToolbar.select_tool()` keeps the toggle-group visuals honest. (5) **Skeleton-era "Connected peers" box removed from the main menu** — dead UI since host/join started navigating straight to the lobby; no pre-lobby linking exists or is planned. Owner also confirmed live: ceremony + titles feel good, M6/cue/reveal all landed ("everything's looking great").

**Status:** [x] Implemented (606 tests green, +3: greyscale-swatch pin, eraser→brush, overlay collapse) [x] Gates PASS ×3 [x] Owner re-check same session: "all 4 landed correctly"

### Polish batch B/C: empty track mask legal, eraser loops, ready-up global, exit button, lobby Esc = Options, borderless chat toggle
**Date:** 2026-08-10 | **Slice:** 21 follow-up (polish B1–B3, C1–C3) | **Decided by:** Owner (polish-notes list + 2 in-session calls) | **Type:** Quick

**Decision:** (1) **B1: `drawing_tracks` mask 0 is now LEGAL** — none selected = silent drawing phase (supersedes TDD 21's never-empty clamp reset; default stays all-on). `_next_music_track()` returns `""` for the empty set; `""` in the ROUND_INTRO payload sets deliberate silence on every peer (the unknown-garbage → ambient fallback is unchanged, and late joiners still default to ambient). (2) **B2: the eraser scrub LOOPS for the whole stroke** — the 1 s WAV imports with `loop_mode=Forward` (chalk scrub loops naturally), lives on a dedicated player OUTSIDE the one-shot SFX map (a looping stream must never enter the pool), started in the canvas ERASER stroke-begin and stopped in `_commit_live_stroke` (the single stroke-end funnel) + phase/scene-change/session-close safety stops. (3) **B3: ready-up clicks are GLOBAL** — `Audio` hooks `ready_state_changed` and plays on set GROWTH only; **unready stays silent (owner call: the sound always means progress toward all-ready)**; the local `click_sfx` metas on Done!/strip became `"none"` (presser would hear it twice). (4) **C1:** Exit-game button, bottom of the main menu. (5) **C2: lobby Esc opens the shared OptionsDialog** — logged deviation from the polish-note's suggested bespoke `LobbyMenu`: volumes are the only per-user settings in existence, and the dialog gives Esc-to-close + the paper skin for free; revisit only if the lobby ever needs more per-user surface. (6) **C3:** chat Hide/Show toggle → `EmojiButton` variation (borderless bare glyph).

**Status:** [x] Implemented (603 tests green: clamp/rotation/audio suites reworked for empty-legal, +silent-drawing case) [x] Gates PASS ×3 [x] Owner ear/feel pass (session-17 playtests; 8-player scale checks remain batchable in qa-backlog §21)
...
**Status:** [x] Sources checked in (`m6-judging.strudel`, `sfx-reveal.strudel`, `cue-timer-warning.strudel` v2) + brief/spec/README updated [x] Rendered (extractor, same day — cue onsets verified at exact 1 s spacing; m6 40.000 s; sfx-reveal 2.29 s) [x] Wired same day (CUE_LANDING_SEC=4.046, M6 map/JUDGING row + db-offset knob, snippet per beat + once on GRID entry; 602 tests, 3 gates) [x] Owner ear pass in-game (session-17 playtests: "felt good" / "everything's looking great")
