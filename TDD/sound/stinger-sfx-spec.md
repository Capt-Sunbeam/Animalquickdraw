...
11. **Card flip/reveal → `sfx-reveal`** (~1 s + ring). **FINAL (owner-approved 2026-08-10 polish session, source `sfx-reveal.strudel`):** "curtain pull" — a fast marimba glitter up two octaves of F pentatonic, landing on an accented F5+F6 marimba octave (S4's landing language — every reveal is a tiny piece of winning). Fires once per one-at-a-time reveal beat. (The original ~120 ms paper-whip design and a 2 s slow version were both rejected — owner wanted a themed reveal gesture at run speed.)
