...
func test_undo_semantics_pop_equals_raster_without_last_op() -> void:
	var with_fill: DrawingDoc = GoldenDocs.stroke_fill()
	var without_last := DrawingDoc.new()
	for i: int in with_fill.ops.size() - 1:
		without_last.ops.append(with_fill.ops[i])
	var popped: DrawingDoc = GoldenDocs.stroke_fill()
	popped.ops.pop_back()
	assert_str(DocRasterizer.image_hash(DocRasterizer.rasterize(popped))) \
...
func test_undo_of_clear_restores_pre_clear_picture() -> void:
	...
	doc.ops.pop_back()  # remove second stroke
	doc.ops.pop_back()  # remove the clear
