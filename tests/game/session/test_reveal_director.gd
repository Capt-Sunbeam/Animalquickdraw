...
func _entry(id: String, stroke_secs: float = 0.0) -> Dictionary:
	var ops: Array = []
	if stroke_secs > 0.0:
...
func test_total_secs_is_beats_plus_gather() -> void:
	var entries: Array[Dictionary] = [_entry("a"), _entry("b", 6.0)]
	...
	var expected: float = RevealDirector.compute_beat_secs(entries[0]["doc"], settings, 2) \
			+ RevealDirector.compute_beat_secs(entries[1]["doc"], settings, 2) \
