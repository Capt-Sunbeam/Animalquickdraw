...
| **Hand-drawn art pass** | **PLANNED + REFINED; PILOT ACTIVE (2026-07-11)** | Style/pipeline/font decisions settled in the 2026-07-11 art discussion session (see decision log): pure ink line art, phone-photo batching, tileable extracted-sticker wallpaper, printerless DIY font (owner prepped 80-box sheets — `TDD/art/font-sheet-guide.md`). Pipeline script built + synthetic-tested: `tools/art_pipeline/clean_scan.py`. **Pilot in flight:** owner draws B1/P1/I1 per `TDD/art/pilot-brief.md` → in-game look-check is the gate. Full pass still slots after Slice 14. Resolved: text tool keeps PixelFont |
